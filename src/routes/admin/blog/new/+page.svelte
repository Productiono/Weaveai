<script lang="ts">
  ...
  let contentMarkdown = $state(form?.values?.contentMarkdown || "");
  let status = $state(form?.values?.status || "draft");
  let featuredImageUrl = $state(form?.values?.featuredImageUrl || "");
  let scheduledFor = $state(form?.values?.scheduledFor || "");
  let metaTitle = $state(form?.values?.metaTitle || "");
  let metaDescription = $state(form?.values?.metaDescription || "");
  let categories = $state(form?.values?.categories || "");
  let tags = $state(form?.values?.tags || "");
...
<div class="space-y-8">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-3xl font-bold tracking-tight">New Blog Post</h1>
      <p class="text-muted-foreground text-sm">
        Draft a new article and publish when ready.
      </p>
    </div>
